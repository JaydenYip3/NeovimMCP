...
function response(value: unknown, structured: Record<string, unknown> = {}): ToolResponse {
    return {
...
async function getVisualSelection(nvim: NeovimClient): Promise<VisualSelection> {
...
type ToolHandler<T = Record<string, unknown>> = (args: T) => Promise<ToolResponse>;
...
export function registerTools(server: McpServer): void {
    ...
    server.registerTool(
        ...
        withErrorHandling(async ({ start, end }: { start: number; end: number }) => {
            if (start >= end) {
                return errorResponse("start must be less than end");
            }
            const nvim = await getNvim();
            const lines = (await nvim.call("getline", [start + 1, end])) as string[];
    ...
    server.registerTool(
        ...
        withErrorHandling(async () => {
            ...
            return response(selection, selection as unknown as Record<string, unknown>);
        }),
    );
